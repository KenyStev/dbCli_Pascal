...
begin
    ...
    begin
        ...
        SetLength(fieldsToShow,Length(fieldsCurrentTable));
        for i := 0 to (Length(fieldsCurrentTable)-1) do
        begin
            write('Show Field: ',fieldsCurrentTable[i].Nombre,'? (1:si/2:no): ');
            readln(option);
            while ((option <> 1) and (option <> 2)) do
            begin
                write('Option not valid! Insert option again: ');
                readln(option);
            end;
            
            if option = 1 
            then fieldsToShow[i] := true
            else fieldsToShow[i] := false;
        end;
        ...
        begin
            if fieldsToShow[i] 
            then write(Format('%10s',[fieldsCurrentTable[i].Nombre]));
        end;
        writeln('');
        ...
        begin
            ...
            begin
                ...
                begin
                    fsOut.Seek(fieldsCurrentTable[i].Size,soCurrent);
                end;
            end;
            writeln('');
